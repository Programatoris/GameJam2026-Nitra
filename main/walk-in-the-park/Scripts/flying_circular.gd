...
func start_floating_movement() -> void:
	...
	tween.tween_property(self, "position:y", position.y - 200, 2.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
	
	# Pohyb späť nadol na pôvodnú pozíciu
	tween.tween_property(self, "position:y", position.y, 2.0).set_trans(Tween.TRANS_SINE).set_ease(Tween.EASE_IN_OUT)
